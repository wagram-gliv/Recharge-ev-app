...
Si le message affiche `Load failed`, cela indique généralement un échec réseau/CORS côté API carto. Le code envoie désormais la requête itinéraire avec `api_key` dans l’URL (sans header `Authorization`) pour éviter les blocages de preflight sur certains endpoints ORS/Heigit.

## Comment "merge le dernier commit sur main" depuis iPhone

Si quelqu'un vous dit "merge le dernier commit sur `main`", voici la traduction en clics GitHub iPhone:

1. Ouvrir **GitHub** > votre repo > **Pull requests**.
2. Appuyer sur **New pull request**.
3. En haut, choisir:
   - **base**: `main`
   - **compare**: la branche où vous avez fait les modifs (souvent `patch-1`, `update-config`, etc.).
4. Vérifier que la page montre bien vos fichiers modifiés (`src/config.js`, `src/main.js`, `src/mapService.js`, etc.).
5. Appuyer sur **Create pull request**.
6. Ouvrir la PR créée, puis appuyer sur **Merge pull request** > **Confirm merge**.
7. Aller sur **Vercel** > **Deployments** > **Redeploy** (without cache).
8. Vérifier que le déploiement "Current" affiche bien le commit de merge récent.

### Si le bouton "Merge pull request" n'apparaît pas
- Soit la PR a des conflits: appuyer sur **Resolve conflicts** (ou faire un nouveau commit via l'éditeur GitHub),
- Soit vous n'êtes pas sur le bon repo/compte,
- Soit votre branche source ne contient aucun changement par rapport à `main`.
